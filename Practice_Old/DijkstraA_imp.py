...
class Node:
    ...
    def __cmp__(self, otherVertex):         #overloading to compare mindistance on a vertex and the distance from source(calculated by traversing edges)
        return self.cmp(self.minDistance, otherVertex.minDistance)

    def __lt__(self, other):                #tell which node is at a higher distance and which is at a lower distance
        selfPriority = self.minDistance
        otherPriority = other.minDistance
        return selfPriority < other.minDistance
...
algorithm = Algorithm();
algorithm.calcShortestPath(vertexList, node1);
